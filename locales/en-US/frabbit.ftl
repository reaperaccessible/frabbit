...
wizard-packages-osara-keymap-preserve-note = For advanced users: your current key map will be preserved. FRABBIT won't touch reaper-kb.ini, you will need to manage staying up to date with the latest OSARA key map additions manually.
wizard-packages-osara-keymap-replace-note = Recommended for new through intermediate users: FRABBIT will backup a copy of your current reaper-kb.ini file, then replace it with the latest version of the OSARA key map.
