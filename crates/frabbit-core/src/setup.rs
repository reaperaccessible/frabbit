...
mod tests {
    ...
    fn keymap_only_install_creates_backup_and_writes_keymap() {
        ...
        assert!(
            backup.exists(),
            "variant-specific backup must exist after keymap-only run"
        );
        assert_eq!(
        ...
        // Old reaper-kb.ini.bak path must not be used any more.
        assert!(
            !resource_path.join("reaper-kb.ini.bak").exists(),
            "legacy reaper-kb.ini.bak must no longer be created"
        );
        assert!(
            kb_ini.exists(),
