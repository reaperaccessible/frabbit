...
main(int argc, char** argv)
{
    ...
    constants_to_extract.push_back(
        { "wxALIGN_CENTER", wxALIGN_CENTER }); // Alias for wxALIGN_CENTRE_HORIZONTAL
    ...
    constants_to_extract.push_back(
        { "wxALIGN_CENTER_VERTICAL",
          wxALIGN_CENTER_VERTICAL }); // Alias for wxALIGN_CENTRE_VERTICAL
    constants_to_extract.push_back({ "wxALIGN_CENTRE_VERTICAL", wxALIGN_CENTRE_VERTICAL });
    constants_to_extract.push_back(
        { "wxALIGN_CENTRE", wxALIGN_CENTRE }); // Combination of horizontal/vertical
    // Sizer Flags
    ...
    constants_to_extract.push_back(
        { "wxTE_PROCESS_TAB", wxTE_PROCESS_TAB }); // Process TAB key in the control
    constants_to_extract.push_back(
        { "wxTE_NOHIDESEL", wxTE_NOHIDESEL }); // Always show selection, even when not focused
    ...
    constants_to_extract.push_back(
        { "wxTE_DONTWRAP", wxTE_DONTWRAP }); // Don't wrap text, show horizontal scrollbar
    ...
    constants_to_extract.push_back(
        { "wxTE_BESTWRAP",
          wxTE_BESTWRAP }); // Wrap at word boundaries or anywhere if word is too long
    constants_to_extract.push_back(
        { "wxTE_CAPITALIZE", wxTE_CAPITALIZE }); // Capitalize first letter (mobile platforms)
    constants_to_extract.push_back(
        { "wxTE_NO_VSCROLL", wxTE_NO_VSCROLL }); // Don't show vertical scrollbar for multiline
    ...
    constants_to_extract.push_back(
        { "wxICON_WARNING", wxICON_WARNING }); // Same as wxICON_EXCLAMATION
    ...
    constants_to_extract.push_back(
        { "wxFLP_SMALL", wxFLP_SMALL }); // Use small version of the button
    ...
    constants_to_extract.push_back(
        { "wxDIRP_SMALL", wxDIRP_SMALL }); // Use small version of the button
    ...
    constants_to_extract.push_back(
        { "wxLB_HSCROLL", wxLB_HSCROLL }); // Note: This is wxListBox specific HSCROLL
    constants_to_extract.push_back(
        { "wxLB_ALWAYS_SB", wxLB_ALWAYS_SB }); // Added: Verify this is a valid wx constant
    ...
    constants_to_extract.push_back(
        { "wxStaticBitmap::Scale_AspectFit", wxStaticBitmap::Scale_AspectFit });
    constants_to_extract.push_back(
        { "wxStaticBitmap::Scale_AspectFill", wxStaticBitmap::Scale_AspectFill });
    // Treebook Styles
    ...
    constants_to_extract.push_back(
        { "wxAUI_NB_BOTTOM",
          wxAUI_NB_BOTTOM }); // Added for completeness, though not in DEFAULT_STYLE
    ...
    constants_to_extract.push_back(
        { "wxWS_CAPTION", wxCAPTION }); // Already defined above but adding for completeness
    ...
    constants_to_extract.push_back(
        { "wxWS_CLIPCHILDREN", wxCLIP_CHILDREN });                          // Already defined above
    ...
    constants_to_extract.push_back(
        { "wxWS_OVERLAPPED", 0x00000000L });                       // Overlapped window (default)
    constants_to_extract.push_back({ "wxWS_POPUP", 0x80000000L }); // Popup window
    constants_to_extract.push_back(
        { "wxWS_SIZEBOX", wxRESIZE_BORDER });                          // Alias for wxRESIZE_BORDER
    ...
    for (const auto& pair : constants_to_extract) {
        ...
        std::cout << "pub const WXD_" << processed_name << ": i64 = " << pair.second << ";"
                  << std::endl;
    }
