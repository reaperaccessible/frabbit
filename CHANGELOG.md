...
- **REAPER n'est plus signalé « en échec » alors qu'il s'est parfaitement installé.** L'installateur silencieux de REAPER installe correctement (fichiers en place, icône sur le Bureau, REAPER fonctionne) mais renvoie un code de sortie non-standard (1223) en fin de course. FRABBIT vérifiait la présence de `reaper.exe` **une fraction de seconde trop tôt** — avant que l'installateur, lancé en tâche élevée, ait fini d'écrire — et concluait à tort à un échec « approbation administrateur refusée ». FRABBIT **re-vérifie désormais la présence des fichiers pendant quelques secondes** avant de conclure : une installation réussie qui rend la main un peu tard est correctement reconnue comme installée. (Le délai n'est payé que par un échec réel ; un succès est reconnu immédiatement.)
